...
GOTCHAS & CRITICAL RULES (AVOID FATAL ERRORS):
  ...
  - NO SPACES IN NODE NAMES: Because quotes are banned, you cannot have spaces in node names either. Use compound words, camelCase, or underscores (e.g., OrganicSearch, Pre_Training, WebVisit).
  - NO TITLES OR STYLING: Do NOT use the title keyword, style, classDef, or inline styling anywhere in the body. Every single line after "sankey-beta" is interpreted strictly as CSV data.
  - NUMERIC VALUES ONLY (NO UNITS / SCIENTIFIC NOTATION): The third column MUST be a plain numeric float or integer (e.g., 500 or 12.5). Never use scientific notation (e.g., 3.3e18), commas in numbers (e.g., 1,000), or appended units (e.g., 50%, $100, 10GB).
  - NO CYCLIC LOOPS / CIRCULAR REFERENCES: Sankey diagrams must be Directed Acyclic Graphs (DAGs). Node flows can ONLY move in one direction. Creating bidirectional flows between nodes (e.g., A -> B AND B -> A) will crash the renderer. Use distinct node names for inputs vs. outputs (e.g., GPU_Hours_In vs. GPU_Hours_Out).
  - PROPORTIONAL SCALE INTEGRITY: Link width is directly proportional to value. Avoid mixing drastically different orders of magnitude (e.g., 1,000,000,000 next to 5). Tiny values relative to the maximum will compress into invisible 0-pixel rendering artifacts or fail layout bounds.
  - NO COMMENT SYNTAX ON DATA LINES: Comments using %% must be on their own line. Do NOT append inline comments on the same line as node CSV data.`,
