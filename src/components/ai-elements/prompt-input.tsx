...
type MenuItemClickHandler = NonNullable<
  ComponentProps<typeof DropdownMenuItem>["onClick"]
>;
type MenuItemClickEvent = Parameters<MenuItemClickHandler>[0];
...
export const PromptInputActionAddAttachments = ({
  ...
  const handleClick = useCallback<MenuItemClickHandler>(
...
export const PromptInputSubmit = ({
  ...
  let iconElement = <Icon icon={"at-icons:arrow-up"} className="size-4" />;

  if (status === "submitted") {
    iconElement = <Icon icon="svg-spinners:ring-resize" className="size-4" />;
  } else if (status === "streaming") {
    iconElement = <Icon icon={"gravity-ui:stop-fill"} className="size-4" />;
  ...
  return (
    <InputGroupButton
      aria-label={isGenerating ? "Stop generating" : "Submit prompt"}
      className={cn(className)}
      onClick={handleClick}
      size={size}
      type="submit"
